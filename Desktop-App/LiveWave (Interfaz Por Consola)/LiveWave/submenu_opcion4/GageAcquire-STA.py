        ...
        try:
            ...
            while True:
                ...
                if status == gc.ACQ_STATUS_READY:
                    break
                elif status != gc.ACQ_STATUS_READY:
...
def debug_raw():
    """
    Función para capturar y guardar datos crudos (raw) sin realizar conversiones.
    """
    sistema = None

    def liberar_recursos():
        if sistema is not None and sistema.handle is not None:
            sistema.liberar()
    ...
    try:
        ...
        with SistemaGage() as sistema:
            try:
                ...
                carpeta_prueba = "prueba_gage"
                carpeta_raw = os.path.join(carpeta_prueba, "raw_data")
                ...
                if not data_dict:
                    logging.warning("No se adquirieron datos.")
                else:
                    # Guardar datos RAW
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S%f")
                    for canal, datos in data_dict.items():
                        # Generar nombre de archivo con timestamp
                        nombre_archivo = os.path.join(carpeta_raw, f"CH{canal}_RAW_{timestamp}.txt")
                        GuardadorDatos.guardar(nombre_archivo, datos)
                        logging.info(f"Datos del canal {canal} guardados en {nombre_archivo}")

            except Exception as e:
                logging.critical(f"Error durante la ejecución principal en debug_raw: {e}")
                traceback.print_exc()
                raise
    except Exception as e:
        logging.critical(f"Fallo crítico en debug_raw: {e}")
        traceback.print_exc()
    finally:
        liberar_recursos()
...
def main():
    """
    Función principal que ejecuta la adquisición de datos crudos para depuración.
    """
    debug_raw()
